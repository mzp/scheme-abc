...
let v_ok variables program =
  let inter_code =
    ...
    ok true @@
      List.for_all (flip InterCode.mem_variable inter_code) variables

let m_ok methods program =
  let inter_code =
    of_program program in
    ok true @@
      List.for_all (flip InterCode.mem_method inter_code) methods

let _ =
  ("interCode.ml" >::: [
     "'define' should export its name" >::
       (fun () ->
	  v_ok ["","x"] [define (`Public (global "x")) (int 42)];
     ...
     "'class' should export its member methods" >::
       (fun () ->
	  let k m =
	    klass (`Public (global "Foo")) (global "Object") [] m in
	    m_ok ["f"]
	      [k [meth "f" [] (int 42)]])
   ]) +> run_test_tt
